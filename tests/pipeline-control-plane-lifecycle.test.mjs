...
test("operational action contracts validate runtime objects without throwing", async () => {
  ...
  assert.equal(readinessCapabilities.get("pause").capabilityState, "gated");
  assert.equal(readinessCapabilities.get("pause").authorityState, "blocked");
  assert.equal(readinessCapabilities.get("pause").typedReason, "blocked_by_policy");
