...
test.describe("dashboard workflow coverage", () => {
  ...
  test("shows compact routing fleet data on controls", async ({ page, request }) => {
    ...
    await expect(runwayPanel.getByText("branch: codex/dispatcher-queue-handoff-audit-download-refresh")).toBeVisible();
    await expect(runwayPanel.getByText('start: node ./scripts/codex-workspace.mjs start "dispatcher queue handoff audit download refresh"')).toBeVisible();
    ...
    await expect(handoffDownloadCard.getByText("branch: codex/dispatcher-queue-handoff-audit-download-refresh")).toBeVisible();
    await expect(handoffDownloadCard.getByText('start: node ./scripts/codex-workspace.mjs start "dispatcher queue handoff audit download refresh"')).toBeVisible();
    await expect(handoffDownloadCard.getByText("pnpm run test:codex-workspace")).toBeVisible();
