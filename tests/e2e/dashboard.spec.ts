...
test.describe("dashboard workflow coverage", () => {
  ...
  test("shows compact routing fleet data on controls", async ({ page, request }) => {
    ...
    await expect(runwayPanel.getByText("branch: codex/dispatcher-closed-source-guard-filter-counts-refresh")).toBeVisible();
    await expect(runwayPanel.getByText('start: node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter counts refresh"')).toBeVisible();
    ...
    await expect(closedSourceGuardFilterCountsCard.getByText("branch: codex/dispatcher-closed-source-guard-filter-counts-refresh")).toBeVisible();
    await expect(closedSourceGuardFilterCountsCard.getByText('start: node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter counts refresh"')).toBeVisible();
    await expect(closedSourceGuardFilterCountsCard.getByText("pnpm run test:e2e:dashboard:controls")).toBeVisible();
    ...
    await expect(runnerAssignmentPanel.getByText("dispatcher-closed-source-guard-filter-counts-refresh: assignable (backlog-assignable) branch codex/dispatcher-closed-source-guard-filter-counts-refresh")).toBeVisible();
    ...
    await expect(runnerAssignmentPanel.getByText("Candidate: dispatcher-closed-source-guard-filter-counts-refresh")).toBeVisible();
    ...
    await expect(runnerAssignmentPanel.locator("article").filter({ hasText: "Dispatcher closed source guard filter counts refresh" }).getByText("source: Backlog")).toBeVisible();
    ...
    await expect(filteredSourceSummary.getByText(/Rows: workspace \d+, lane assignment \d+, backlog \d+/)).toBeVisible();
    await expect(filteredSourceSummary.getByText(/Source completion: assignment \d+, workspace \d+, none \d+/)).toBeVisible();
    await assignmentRowFilters.getByRole("button", { name: "Show assignment-backed rows" }).click();
    await expect(resetAssignmentFilters).toBeEnabled();
    ...
    await assignmentRowFilters.getByRole("button", { name: "Show workspace-backed rows" }).click();
    ...
    await expect(filteredSourceSummary.getByText("Source completion: assignment 0, workspace 1, none 0", { exact: true })).toBeVisible();
    await assignmentRowFilters.getByRole("button", { name: "Show uncompleted rows" }).click();
