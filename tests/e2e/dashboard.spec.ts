...
type SupervisorPipelinePacketSnapshot = { status: string; updatedAt: string };

function readSupervisorPipelinePacket(packetId: string) {
...
function restoreSupervisorPipelinePacket(packetId: string, snapshot: SupervisorPipelinePacketSnapshot) {
  ...
  const script = [
    ...
    "conn.commit()",
    "restored = conn.execute(\"select status, updated_at from authoritative_work_packets where id = ?\", (packet_id,)).fetchone()",
    "assert restored is not None and restored[0] == status and restored[1] == updated_at",
    "conn.close()",
...
test.describe("dashboard workflow coverage", () => {
  ...
  if (process.env.PLAYWRIGHT_EXPECT_EMPTY_SUPERVISOR === "true") {
    test("Story 4.6 empty runtime state does not substitute demo packets", async ({ page }) => {
      ...
      await expect(truthSummary.getByText("Supervisor returned zero persisted WorkPacketV0 rows", { exact: false })).toBeVisible();
  ...
  test("opens isolated demo pipeline cockpit without live execution framing", async ({ page, baseURL }, testInfo) => {
    ...
    await expect(commandStrip.getByLabel("Top blocked packet: Pause runner loop at limit window", { exact: true })).toBeVisible();
    ...
    const deliveryEvidence = deliveryPacketDetail.locator("article").filter({ hasText: "fixture:deliver-evidence:delivery-cleanup-evidence" });
    await expect(deliveryEvidence.getByText("metadata_only", { exact: true })).toBeVisible();
    await expect(deliveryEvidence.getByText("Cleanup dry-run", { exact: true })).toBeVisible();
    await expect(deliveryEvidence.getByText("Cleanup target", { exact: true })).toBeVisible();
    await expect(deliveryEvidence.getByText("Delivery rails grant authority", { exact: true })).toBeVisible();
    await expect(deliveryEvidence.getByText("Remote mutation approved", { exact: true })).toBeVisible();
    await expect(deliveryEvidence.getByText("Merge approved", { exact: true })).toBeVisible();
  ...
  test("shows compact routing fleet data on controls", async ({ page, request }) => {
    ...
    await expect(verificationPanel.getByText("pnpm run test:manager-quality-gate", { exact: true })).toBeVisible();
    ...
    await expect(runwayPanel.getByText("Completed dispatcher continuity and report navigation work now remains closed evidence rather than dispatchable backlog capacity.")).toBeVisible();
