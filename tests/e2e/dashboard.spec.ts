...
test.describe("dashboard workflow coverage", () => {
  ...
  test("shows compact routing fleet data on controls", async ({ page, request }) => {
    ...
    await expect(runwayPanel.getByText("branch: codex/dispatcher-closed-lane-requeue-guard-refresh")).toBeVisible();
    await expect(runwayPanel.getByText('start: node ./scripts/codex-workspace.mjs start "dispatcher closed lane requeue guard refresh"')).toBeVisible();
    ...
    await expect(closedLaneRequeueGuardCard.getByText("branch: codex/dispatcher-closed-lane-requeue-guard-refresh")).toBeVisible();
    await expect(closedLaneRequeueGuardCard.getByText('start: node ./scripts/codex-workspace.mjs start "dispatcher closed lane requeue guard refresh"')).toBeVisible();
    await expect(closedLaneRequeueGuardCard.getByText("pnpm run test:e2e:dashboard:controls")).toBeVisible();
    ...
    await expect(runnerAssignmentPanel.getByText("dispatcher-closed-lane-requeue-guard-refresh: assignable (backlog-assignable) branch codex/dispatcher-closed-lane-requeue-guard-refresh")).toBeVisible();
    ...
    await expect(runnerAssignmentPanel.getByText("Candidate: dispatcher-closed-lane-requeue-guard-refresh")).toBeVisible();
    ...
    await expect(runnerAssignmentPanel.locator("article").filter({ hasText: "Dispatcher closed lane requeue guard refresh" }).getByText("source: Backlog")).toBeVisible();
