...
test("work item detail surfaces persisted memory proposal review controls", async () => {
  ...
  assert.match(panelSource, /expectedRevision: proposal\.revision/);
  ...
  assert.match(panelSource, /function hasLlmWikiArtifact\(proposal: DashboardCanonicalMemoryProposalV1\): boolean \{\s+return proposal\.llmWikiArtifactSearchEligible;\s+\}/);
  ...
  assert.match(panelSource, /proposal\.aiDraftEligible/);
  assert.doesNotMatch(panelSource, /targetsAiDraftQueue/);
