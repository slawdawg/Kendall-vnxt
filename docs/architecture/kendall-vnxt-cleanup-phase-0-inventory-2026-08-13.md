...
| Epic/story-specific report routes and panels | **classify then retire** | Many supervisor report routes serve readiness, maintenance, Git hygiene, or dated epic concerns. | Classification complete in [supervisor report surface classification](kendall-vnxt-supervisor-report-surface-classification-2026-08-15.md); perform family-by-family migration only after consumer and replacement proof. |
...
| Lifecycle truth has an initial canonical compatibility slice. | PR [#822](https://github.com/slawdawg/Kendall-vnxt/pull/822), merged as `dabe3928`, projects canonical packet list/detail data through the dashboard runtime. Detail fallback is restricted to the two explicit legacy identities; malformed, 404, or legacy-shaped canonical **list** responses retain the existing broad legacy-list fallback. | Phase 2 owns removal of the remaining V0/legacy surface, including the broad list fallback. |
...
| Closeout delivery-harness liveness is recorded rather than masked. | The managed closeout verifier completed its preceding stages but exceeded its 900-second boundary in the `test:codex-workspace` fixture process; no source or documentation behavior failure was reported. | Phase 5 owns deterministic fixture diagnostics and an equivalence-backed repair. Do not record this aggregate verifier as green or merely extend its timeout without that evidence. |
