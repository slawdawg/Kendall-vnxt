...
| `/pipeline/demo` and its fixture catalogue | **decision resolved: retire unless it is needed for an isolated demo package** | Fixture and test surfaces are substantial; its product status is not demonstrated by current authority docs. | Inventory route/proxy callers; remove route, catalogue, and tests together unless an isolated package is retained. |
