...
read-model slice replaced normal dashboard list and packet-detail reads with a
dashboard-owned `DashboardCanonicalWorkPacketV1`: it requests only canonical
list/detail data and carries the Python-only extensions under explicit
validation. The former read-only `compatibilityProjection` adapter was removed
by Slice5A/B; this historical slice did not remove `/work-packets` or its
transports for then-unmigrated callers.
...
| --- | --- | --- |
| Shared contracts: [`pipeline-control-plane/index.ts`](../../packages/contracts/src/pipeline-control-plane/index.ts) and [`work-packet.ts`](../../packages/contracts/src/work-packet.ts) | The route-only V0 list/detail envelopes are retired. `WorkPacketV0View`, lifecycle-state, evidence, and dashboard compatibility types remain internal holds; the shared lifecycle core still does not contain the Python-only canonical extensions. | Retain only explicitly isolated compatibility adapters until all V0 imports are gone. Add compile-time and runtime contract coverage for every included, projected, or intentionally omitted extension. |
| Supervisor API and application: [`main.py`](../../services/supervisor/src/supervisor/api/main.py), [`schemas.py`](../../services/supervisor/src/supervisor/api/schemas.py), and [`service.py`](../../services/supervisor/src/supervisor/application/service.py) | Canonical create/list/detail/transition/local-proof routes remain; legacy `GET /work-packets` list/detail reads and their service helper are retired with source-zero, direct-404, proxy-denial, Gate4, and restart-readback proof. Application service still materializes V0 views only for the explicit internal projection hold. | Keep canonical routes stable while later slices retire the remaining internal V0 projection dependencies. |
...
| Dashboard runtime and pages: [`pipeline-supervisor-runtime.ts`](../../apps/dashboard/src/lib/pipeline-supervisor-runtime.ts), [`pipeline-packet-loader.ts`](../../apps/dashboard/src/lib/pipeline-packet-loader.ts), and work-item pages/components | `DashboardCanonicalWorkPacketV1` carries the validated authoritative lifecycle plus the Python-only `canonicalContract`, `evidenceChain`, and `productModeMapping` extensions. Its former `compatibilityProjection` adapter is retired; WorkItem detail URLs resolve through the persisted `WorkItem.authoritative_packet_id` canonical lookup, never a synthetic `work_item:` legacy read. | Preserve list, direct-detail, and WorkItem lookup no-legacy-request proofs. An absent WorkItem mapping or malformed canonical lookup is displayed as an unavailable packet state; malformed present extensions fail closed. Remaining V0 holds are nested service/schema/database/demo/direct-detail boundaries. |
| Dashboard V0 projections and UI: `active-board-view-model.ts`, `memory-proposal-review-panel.tsx`, and `pipeline-fixtures.ts` | The dashboard TS V0 projector/validator and top-level re-export were retired in Slice5B. The fixture catalog, nested V0 values, supervisor materializer, and direct-detail evidence remain explicit compatibility/detail holds; normal cockpit rendering uses canonical packet identities and the V1 active-board model. | Keep fixture support explicitly fixture-only and direct-detail evidence bounded; retire each remaining V0 service/schema/database hold only with its own replacement and proof. |
| Dashboard transport and allowlists: [`dashboard-supervisor-proxy.mjs`](../../apps/dashboard/scripts/dashboard-supervisor-proxy.mjs), [`pipeline-supervisor-uds.ts`](../../apps/dashboard/src/lib/pipeline-supervisor-uds.ts), and [`dashboard-page-read-manifest.json`](../../apps/dashboard/src/lib/dashboard-page-read-manifest.json) | The proxy/UDS retain canonical packet and exact WorkItem reads; legacy packet paths are no longer admitted. | Keep the exact canonical lookup with focused tests. Legacy list/detail denial tests must remain fail-closed and not forward a request. |
| Operational scripts and tests: [`pipeline_operational_smoke.py`](../../services/supervisor/scripts/pipeline_operational_smoke.py), [`test_work_packets.py`](../../services/supervisor/tests/integration/test_work_packets.py), [`gate4-bmad-dashboard-e2e.mjs`](../../scripts/gate4-bmad-dashboard-e2e.mjs), and [`check-dashboard-pipeline-import-boundary.mjs`](../../scripts/check-dashboard-pipeline-import-boundary.mjs) | Canonical lifecycle behavior, native WorkItem/CandidateWork persistence, direct legacy-404, Gate4 canonical restart proof, and Slice5B dashboard source-zero checks cover the retired HTTP and TS projector surfaces. | Keep source-zero and replacement assertions; later work is limited to the remaining internal service/schema/database holds. |
...
| Dashboard read model | Supervisor canonical list/detail envelope | `pipeline-packet-loader` keeps `DashboardCanonicalWorkPacketV1` server-side and carries client-safe canonical lifecycle metadata plus the V1 active-board model through normal and LAN cockpit source state. Raw lifecycle payload summaries, evidence refs, Python-only extension records, and projection `canonicalContract`/`productModeMapping` values do not cross the client boundary. The normal cockpit no longer consumes a V1-to-V0 compatibility adapter; explicit V0 direct-detail and fixture holds remain separately named. |
...
| Legacy V0 data | Transitional compatibility only | The dashboard TS V0 projector/validator are retired. Remaining V0-only boundaries are explicit demo fixtures, nested V0 contracts/service materializer/database values, direct-detail evidence, and the two WorkItem detail consumers (`app/work-items/[work-item-id]/page.tsx` and `components/work-item-detail-page.tsx`). Direct pipeline packet detail carries its canonical DTO to `PacketDetailPage`, which owns its named read-only adapter. Normal cockpit callers carry canonical identities and the V1 active-board model without a V0-shaped adapter. No new V0 field, route, fixture-backed production fallback, or parallel parity logic is allowed. |
...
2. **Dashboard without fallback:** dashboard list and detail consume the
   ...
   return a bare `WorkPacketV0View`; the former cockpit `compatibilityProjection`
   adapter is retired, while `PacketDetailPage`'s named direct-detail hold, the two WorkItem detail consumers
