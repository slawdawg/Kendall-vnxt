...
| Dashboard read model | Supervisor canonical list/detail envelope | `pipeline-packet-loader` carries `DashboardCanonicalWorkPacketV1` through normal and LAN cockpit source state. The cockpit consumes its named `compatibilityProjection` only at the remaining V0 projection boundary; it does not initiate a legacy fallback. |
