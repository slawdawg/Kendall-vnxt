...
| Branch foundation setup | `docs/workflows/branch-foundation-setup.md`, `scripts/branch-foundation.mjs` usage text, `pnpm run check:branch-foundation` | Local or remote `dev`, `staging`, `main`, or `prod` state is missing, dirty, unknown, or being mutated |
