...
| --- | --- | --- |
| `apps/dashboard/src/lib/pipeline-fixtures.ts` | Imported by `/pipeline/demo` and `/pipeline/demo/packets/[packetId]`. The demo is the explicit fixture catalog, now emitted as the strict `dashboard-pipeline-fixture/v1` DTO with a fixture-only detail extension; normal `/pipeline` is tested to avoid this import. | Retain. Removing it would break the supported demo and its fixture-only contract. Nested V0 service/schema/projector/database holds remain separately inventoried. |
