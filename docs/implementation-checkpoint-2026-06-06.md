...
## Live Remote-Delivery Readiness

Progress:

- Remote delivery now checks live-target readiness before exposing the executable supervisor action.
- The policy ledger blocks remote delivery until both GitHub auth and an origin remote are available.
- Integration coverage now proves both paths: the live remote-delivery smoke succeeds when the target is ready, and the policy ledger blocks the action when auth is missing.

Verification:

- `uv run --directory services/supervisor pytest tests/integration/test_supervisor_flow.py -q -k remote_delivery`: 3 passed.
- `pnpm run check`: passed.
- `pnpm run test:e2e:dashboard`: 12 passed.

Current status:

- The supervisor can now decide whether a live remote delivery is actually ready before advertising the action to operators.
- The remote-delivery path has now been proven against a real GitHub PR, GitHub Actions check run, and merge on the private repository via disposable empty-commit smoke branch `codex-remote-smoke-20260606-191535` and PR `#9`.
- That external smoke completed end to end with `gh pr checks --watch --fail-fast` passing and `gh pr merge --squash --delete-branch` succeeding.
