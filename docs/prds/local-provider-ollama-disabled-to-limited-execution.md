...
- Should Ollama use a provider-specific setting separate from the existing broad local provider flag?
- Which default model id is acceptable for local evidence explanation?
- Should response summaries be generated by deterministic truncation or a second bounded summarization pass?
- What is the maximum retained response size before redaction/truncation?
