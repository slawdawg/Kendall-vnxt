...
| `linux-install-mvp.md` | Delivered lane | Linux Install MVP planning, implementation docs, fresh-host proof, rerun evidence, package refresh, PR delivery, and merge completed through PR #144; the post-merge closeout lane adds delivery-state hardening and a retrospective draft synthesis while the source PRD file intentionally retains draft metadata until a PRD finalization workflow runs, and completed worktree cleanup remains bounded by its documented authority gate. |
