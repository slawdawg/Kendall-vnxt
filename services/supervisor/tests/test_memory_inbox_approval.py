...
async def test_approval_locks_exact_ready_revision_and_enqueues_each_source_copy_once(tmp_path) -> None:
    ...
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        ...
        session.add_all((source, source_revision, proposal, proposal_revision, MemoryInboxManifest(id="manifest:one", owner_revision_id=source_revision.id, copy_class="quarantine", store_ref="inbox-store:approval-one", creation_state="Created", retention_class="source_retention", deletion_state="None"), MemoryInboxManifest(id="manifest:two", owner_revision_id=source_revision.id, copy_class="preview", store_ref="inbox-store:approval-two", creation_state="Created", retention_class="source_retention", deletion_state="None")))
        ...
        assert approved.deletion_operations == 2 and replay.replayed
