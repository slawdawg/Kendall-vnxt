...
def test_development_runway_report_groups_larger_safe_slices_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert report_slice["includedBacklogItems"] == ["dispatcher-closed-source-guard-filter-counts-refresh"]
    assert "verify-evidence-surfaces" in report_slice["includedActionSteps"]
    assert report_slice["nextLane"]["laneSlug"] == "dispatcher-closed-source-guard-filter-counts-refresh"
    assert report_slice["nextLane"]["branchName"] == "codex/dispatcher-closed-source-guard-filter-counts-refresh"
    assert report_slice["nextLane"]["startCommand"] == 'node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter counts refresh"'
...
def test_safe_development_backlog_report_prioritizes_large_safe_slices_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert dispatcher_closed_source_guard_filter_counts_item["status"] == "ready"
    assert dispatcher_closed_source_guard_filter_counts_item["nextLane"]["branchName"] == "codex/dispatcher-closed-source-guard-filter-counts-refresh"
    assert dispatcher_closed_source_guard_filter_counts_item["nextLane"]["startCommand"] == 'node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter counts refresh"'
    assert "pnpm run test:e2e:dashboard:controls" in dispatcher_closed_source_guard_filter_counts_item["nextLane"]["verificationCommands"]
...
def test_runner_assignment_status_report_reads_claimed_assignment_records(tmp_path, monkeypatch) -> None:
    ...
    assert continuity["selectedBacklogItemId"] == "dispatcher-closed-source-guard-filter-counts-refresh"
    assert continuity["selectedBranch"] == "codex/dispatcher-closed-source-guard-filter-counts-refresh"
    ...
    assert dispatcher_closed_source_guard_filter_counts_backlog["classification"] == "assignable"
    assert dispatcher_closed_source_guard_filter_counts_backlog["reasonCode"] == "backlog-assignable"
    ...
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-counts-refresh"]["classification"] == "assignable"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-counts-refresh"]["reasonCode"] == "backlog-assignable"
...
def test_runner_assignment_status_report_closes_stale_ready_items_with_source_completion_evidence(tmp_path, monkeypatch) -> None:
    ...
    assignments_dir.mkdir(parents=True)
    tasks_dir.mkdir()
    assignments_dir.joinpath("dispatcher-closed-source-guard-filter-counts-refresh.json").write_text(
        json.dumps(
            {
                "assignment_id": "dispatcher-closed-source-guard-filter-counts-refresh",
                "task_id": "20260623-dispatcher-closed-source-guard-filter-counts-refresh",
                "lane_slug": "dispatcher-closed-source-guard-filter-counts-refresh",
                "branch": "codex/dispatcher-closed-source-guard-filter-counts-refresh",
                "status": "closed",
                "owner": "runner-a",
                "phase": "closed",
                "closed_at": datetime.now(timezone.utc).isoformat(),
                "updated_at": datetime.now(timezone.utc).isoformat(),
                "source_backlog_item": {
                    "item_id": "dispatcher-closed-source-guard-filter-counts-refresh",
                    "status": "ready",
                    "branch_name": "codex/dispatcher-closed-source-guard-filter-counts-refresh",
                },
            }
        )
    )
    ...
    assert report["sourceCompletionRollup"] == {
        ...
        "assignment": 1,
        "workspace": 1,
        "sourceBacklogItemIds": ["read-only-evidence-polish", "dispatcher-closed-source-guard-filter-counts-refresh"],
    }
    filter_counts_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-counts-refresh")
    assert filter_counts_backlog["classification"] == "closed"
    assert filter_counts_backlog["reasonCode"] == "backlog-closed-source-assignment"
    assert filter_counts_backlog["reason"] == "closed assignment evidence exists for dispatcher-closed-source-guard-filter-counts-refresh"
    assert filter_counts_backlog["nextSafeAction"] == "Use closed source completion evidence only; choose the next ready safe backlog lane"
    assert filter_counts_backlog["sourceCompletionEvidence"] == {
        "evidenceKind": "assignment",
        "recordId": "dispatcher-closed-source-guard-filter-counts-refresh",
        "sourceBacklogItemId": "dispatcher-closed-source-guard-filter-counts-refresh",
        "branch": "codex/dispatcher-closed-source-guard-filter-counts-refresh",
        "taskId": "20260623-dispatcher-closed-source-guard-filter-counts-refresh",
        "sourceAssignmentId": None,
        "evidencePath": (assignments_dir / "dispatcher-closed-source-guard-filter-counts-refresh.json").as_posix(),
        "evidenceSummary": "Closed assignment record dispatcher-closed-source-guard-filter-counts-refresh matches source backlog item dispatcher-closed-source-guard-filter-counts-refresh.",
    }
