...
def test_safe_development_backlog_report_prioritizes_large_safe_slices_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert queue_zero_buffer_refresh["status"] == "ready"
    assert queue_zero_buffer_refresh["recommendedSliceSize"] == "medium_to_large"
    assert queue_zero_buffer_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-buffer-refresh"
    assert queue_zero_buffer_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-buffer-refresh"
    assert any(
        "closed codex/queue-zero-runway-standby-refresh" in stop_line for stop_line in queue_zero_buffer_refresh["nextLane"]["stopLines"]
    )
    ...
    assert any("worker launch" in evidence for evidence in queue_zero_buffer_refresh["evidence"])
    assert any("provider calls" in evidence for evidence in queue_zero_buffer_refresh["evidence"])
    assert any("premium execution" in evidence for evidence in queue_zero_buffer_refresh["evidence"])
    assert any("process authority" in evidence for evidence in queue_zero_buffer_refresh["evidence"])
...
def test_runner_assignment_status_report_reads_claimed_assignment_records(tmp_path, monkeypatch) -> None:
    ...
    assert continuity["selectedBacklogItemId"] == "queue-zero-runway-buffer-refresh"
    assert continuity["selectedBranch"] == "codex/queue-zero-runway-buffer-refresh"
    ...
    assert queue_proof_rows["queue-zero-runway-buffer-refresh"]["classification"] == "assignable"
    ...
    assert queue_zero_runway_assignable == ["queue-zero-runway-buffer-refresh"]
