...
def test_execution_configuration_checks_report_disabled_defaults_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert checks["ollama-provider-gate"]["disabledReason"] == "ollama_enablement_authority_unresolved"
...
def test_ollama_provider_gate_stays_non_executing_when_broad_gate_is_enabled(tmp_path, monkeypatch) -> None:
    ...
    assert ollama["registryState"] == "enablement_authority_unresolved"
...
def test_ollama_settings_default_provider_and_automatic_gates_false(monkeypatch) -> None:
    for name in (
        ...
        "SUPERVISOR_OLLAMA_APPROVED_SOURCE_VM",
    ):
    ...
    assert settings.ollama_approved_source_vm is None
    ...
    assert state["enablement_approved"] is False
    assert state["disabled_reason"] == "ollama_enablement_authority_unresolved"
...
def test_ollama_provider_gate_requires_approved_endpoint_before_execution(tmp_path, monkeypatch) -> None:
    ...
    assert ollama["adapterReady"] is False
    assert ollama["registryState"] == "enablement_authority_unresolved"
    assert ollama["disabledReason"] == "ollama_enablement_authority_unresolved"
    assert ollama["httpCallsAttempted"] is False
    assert ollama["modelCallsAttempted"] is False


@pytest.mark.parametrize("source_vm", ["192.168.1.118", "192.168.1.8"])
def test_ollama_provider_gate_holds_both_source_vm_candidates_despite_exact_endpoint_and_model(
    tmp_path,
    monkeypatch,
    source_vm,
) -> None:
    ...
    policy["status"] = "hold_conflicting_source_vm"
    policy["approvedSourceVm"] = None
    ...
    ollama_state = service._ollama_provider_gate_state()
    checks = {check.checkId: check for check in service.get_execution_configuration_checks().checks}
    proofs = {proof.workerId: proof for proof in service.list_disabled_provider_proofs()}

    assert ollama_state["authority_status"] == "hold_conflicting_source_vm"
    assert ollama_state["authority_resolved"] is False
    assert ollama_state["enabled"] is False
    assert ollama_state["disabled_reason"] == "ollama_authority_policy_unresolved"
    assert checks["ollama-provider-gate"].enabled is False
    assert checks["ollama-provider-gate"].disabledReason == "ollama_authority_policy_unresolved"
    assert checks["ollama-provider-gate"].providerCallsAllowed is False
    assert checks["ollama-provider-gate"].modelCallsAllowed is False
    assert "Agreed endpoint metadata: http://192.168.1.128:11434/v1/chat/completions." in checks["ollama-provider-gate"].evidence
    assert "Agreed model metadata: qwen3:14b." in checks["ollama-provider-gate"].evidence
...
def test_ollama_provider_gate_ignores_runtime_authority_policy_path_override(tmp_path, monkeypatch) -> None:
    unreviewed_policy_path = tmp_path / "unreviewed-local-provider-authority-policy-v1.json"
    policy = json.loads(
        (Path(__file__).resolve().parents[4] / "docs/workflows/local-provider-authority-policy-v1.json").read_text(
            encoding="utf-8"
        )
...
def test_ollama_provider_gate_consumes_a_complete_reviewed_authority_policy(tmp_path, monkeypatch) -> None:
    ...
    policy["route"]["connectTimeoutSeconds"] = 2.0
    ...
    invalid_state = SupervisorService(Settings(), EventBus())._ollama_provider_gate_state()
    assert invalid_state["disabled_reason"] == "ollama_authority_policy_invalid"
    del policy["unvalidatedExtra"]
    policy["route"]["unreviewedEndpoint"] = "http://127.0.0.1:11434/v1/chat/completions"
    policy_path.write_text(json.dumps(policy), encoding="utf-8")
    invalid_state = SupervisorService(Settings(), EventBus())._ollama_provider_gate_state()
    assert invalid_state["disabled_reason"] == "ollama_authority_policy_invalid"
    del policy["route"]["unreviewedEndpoint"]
...
def test_ollama_provider_gate_fails_closed_when_authority_policy_parser_recurses(monkeypatch) -> None:
    ...
    def raise_recursion(*_args, **_kwargs):
        raise RecursionError("authority policy nesting exhausted")

    monkeypatch.setattr(service_module.json, "loads", raise_recursion)

    policy = service_module._load_local_provider_authority_policy()
...
def test_ollama_local_evidence_explanation_requires_instance_approval_before_adapter_call(tmp_path, monkeypatch) -> None:
    ...
    assert explanation["providerAttempt"]["rejectionReason"] == "ollama_enablement_authority_unresolved"

    events = events_response.json()["data"]
    recorded = next(event for event in events if event["eventType"] == "routing.local_evidence_explained")
    assert recorded["payload"]["providerAttempt"]["rejectionReason"] == "ollama_enablement_authority_unresolved"
...
def test_ollama_local_evidence_explanation_creates_no_automatic_approval_for_unapproved_source_vm(
    ...
    adapter_calls = {"count": 0}
    automatic_approval_calls = {"count": 0}
    ...
    monkeypatch.setattr(service, "_automatic_local_provider_approval", fake_automatic_approval)

    profile = RoutingProfileView(
        workItemId="work-item-authority-hold",
        ...
        preferredLanes=["local_readonly"],
        forbiddenLanes=[],
        escalationTriggers=[],
    )
    decision = RoutingDecisionView(
        decisionId="decision-authority-hold",
        workItemId="work-item-authority-hold",
        stepId="evidence-step",
        createdAt=datetime.now(timezone.utc),
        profileSnapshot=profile,
        selectedLane="local_readonly",
        selectedWorkerId="local.readonly.mock",
        authorityMode="read_only",
        confidenceScore=1.0,
        confidenceBand="high",
        reasonCodes=["bounded-local-evidence"],
        rejectedLanes=[],
        rejectedWorkers=[],
        permissionSummary="Read only.",
        escalationPath=[],
        humanExplanation="Bounded local evidence.",
    )
    preview = RoutingPreviewView(profile=profile, decision=decision)
    item = SimpleNamespace(
        id="work-item-authority-hold",
        title="Authority hold",
        requested_outcome="No provider call.",
        blocked_reason=None,
    )
    ...
    async def acquire_lock(_session):
        return None

    async def routing_preview(_session, _work_item_id, _payload):
        return preview

    async def events(_session, _work_item_id):
        return []

    monkeypatch.setattr(service, "_acquire_execute_admission_lock", acquire_lock)
    monkeypatch.setattr(service, "get_routing_preview", routing_preview)
    monkeypatch.setattr(service, "list_work_item_events", events)

    explanation = asyncio.run(
        service.get_local_evidence_explanation(
            Session(),
            item.id,
            WorkItemLocalEvidenceExplanationRequest(taskKind="evidence_summary", recordEvent=False),
        )
    )
    ...
    assert explanation.providerAttempt.rejectionReason == "ollama_enablement_authority_unresolved"
    assert service._ollama_provider_gate_state()["disabled_reason"] == "ollama_enablement_authority_unresolved"
...
def test_ollama_local_evidence_explanation_accepts_exact_approval_after_authority_decision(tmp_path, monkeypatch) -> None:
    ...
    captured_provider_prompt = {"count": 0, "reservation": None}

    async def fake_explain(self, *, evidence_summary, evidence_count, cancellation_event=None):
        captured_provider_prompt["count"] += 1
...
def test_maintenance_readiness_report_tracks_safe_work_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert blocker_track["status"] == "ready"
    assert (
    ...
    assert "Subscription-agent, Codex CLI, and Claude CLI process launch remain disabled for this readiness slice." in blocker_track["evidence"]
    ...
    assert any("must not approve runtime local provider/model calls" in stop_line for stop_line in report["stopLines"])
...
def test_safe_development_backlog_report_prioritizes_large_safe_slices_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert queue_zero_item["status"] == "closed"
    assert queue_zero_item["recommendedSliceSize"] == "complete"
    assert queue_zero_item["nextLane"]["laneSlug"] == "queue-zero-runway-continuity-refresh"
    ...
    assert queue_zero_followup["status"] == "closed"
    assert queue_zero_followup["recommendedSliceSize"] == "complete"
    assert queue_zero_followup["nextLane"]["laneSlug"] == "queue-zero-runway-successor-refresh"
    assert queue_zero_followup["nextLane"]["branchName"] == "codex/queue-zero-runway-successor-refresh"
    assert "completed runway evidence only" in queue_zero_followup["nextAction"]
    assert "without runtime execution" in queue_zero_followup["nextAction"]
    assert "metadata-only" in queue_zero_followup["nextAction"]
    queue_zero_successor_refresh = next(item for item in report["items"] if item["itemId"] == "queue-zero-runway-successor-refresh")
    assert queue_zero_successor_refresh["status"] == "closed"
    assert queue_zero_successor_refresh["recommendedSliceSize"] == "complete"
    assert queue_zero_successor_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-replenishment-refresh"
    assert queue_zero_successor_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-replenishment-refresh"
    assert "completed runway evidence only" in queue_zero_successor_refresh["nextAction"]
    assert "without runtime execution" in queue_zero_successor_refresh["nextAction"]
    queue_zero_replenishment_refresh = next(item for item in report["items"] if item["itemId"] == "queue-zero-runway-replenishment-refresh")
    assert queue_zero_replenishment_refresh["status"] == "closed"
    assert queue_zero_replenishment_refresh["recommendedSliceSize"] == "complete"
    assert queue_zero_replenishment_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-reserve-refresh"
    assert queue_zero_replenishment_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-reserve-refresh"
    ...
    assert queue_zero_standby_refresh["status"] == "closed"
    assert queue_zero_standby_refresh["recommendedSliceSize"] == "complete"
    assert queue_zero_standby_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-buffer-refresh"
    assert queue_zero_standby_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-buffer-refresh"
    assert "completed runway evidence only" in queue_zero_standby_refresh["nextAction"]
    assert "without runtime execution" in queue_zero_standby_refresh["nextAction"]
    queue_zero_buffer_refresh = next(item for item in report["items"] if item["itemId"] == "queue-zero-runway-buffer-refresh")
    assert queue_zero_buffer_refresh["status"] == "closed"
    assert queue_zero_buffer_refresh["recommendedSliceSize"] == "complete"
    assert queue_zero_buffer_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-overflow-refresh"
    assert queue_zero_buffer_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-overflow-refresh"
    ...
    assert queue_zero_spillover_refresh["status"] == "closed"
    assert queue_zero_spillover_refresh["recommendedSliceSize"] == "complete"
    assert queue_zero_spillover_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-carryover-refresh"
    assert queue_zero_spillover_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-carryover-refresh"
    assert any(
        "closed codex/queue-zero-runway-spillover-refresh" in stop_line for stop_line in queue_zero_spillover_refresh["nextLane"]["stopLines"]
    )
    assert "completed runway evidence only" in queue_zero_spillover_refresh["nextAction"]
    assert "without runtime execution" in queue_zero_spillover_refresh["nextAction"]
    queue_zero_carryover_refresh = next(item for item in report["items"] if item["itemId"] == "queue-zero-runway-carryover-refresh")
    assert queue_zero_carryover_refresh["status"] == "closed"
    assert queue_zero_carryover_refresh["recommendedSliceSize"] == "complete"
    assert any(
        "closed codex/queue-zero-runway-spillover-refresh" in stop_line for stop_line in queue_zero_carryover_refresh["nextLane"]["stopLines"]
    )
    assert queue_zero_carryover_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-relay-refresh"
    assert queue_zero_carryover_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-relay-refresh"
    assert "completed runway evidence only" in queue_zero_carryover_refresh["nextAction"]
    assert "without runtime execution" in queue_zero_carryover_refresh["nextAction"]
    queue_zero_relay_refresh = next(item for item in report["items"] if item["itemId"] == "queue-zero-runway-relay-refresh")
    assert queue_zero_relay_refresh["status"] == "ready"
    assert queue_zero_relay_refresh["recommendedSliceSize"] == "medium_to_large"
    assert queue_zero_relay_refresh["nextLane"]["laneSlug"] == "queue-zero-runway-relay-refresh"
    assert queue_zero_relay_refresh["nextLane"]["branchName"] == "codex/queue-zero-runway-relay-refresh"
    assert any("closed codex/queue-zero-runway-carryover-refresh" in stop_line for stop_line in queue_zero_relay_refresh["nextLane"]["stopLines"])
    assert any("closed codex/queue-zero-runway-spillover-refresh" in stop_line for stop_line in queue_zero_relay_refresh["nextLane"]["stopLines"])
    assert "metadata-only" in queue_zero_relay_refresh["nextAction"]
    ...
    assert any("worker launch" in evidence for evidence in queue_zero_relay_refresh["evidence"])
    assert any("provider calls" in evidence for evidence in queue_zero_relay_refresh["evidence"])
    assert any("premium execution" in evidence for evidence in queue_zero_relay_refresh["evidence"])
    assert any("process authority" in evidence for evidence in queue_zero_relay_refresh["evidence"])
    ...
    assert queue_proof_item["status"] == "closed"
    assert queue_proof_item["recommendedSliceSize"] == "complete"
    assert queue_proof_item["nextLane"] is None
    assert "do not requeue assignment-report-queue-proof-refresh" in queue_proof_item["nextAction"]
    assert "GET /supervisor/runner-assignment-status-report" in queue_proof_item["relatedReports"]
    dispatcher_fixture_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-state-fixtures-refresh")
    assert dispatcher_fixture_item["status"] == "closed"
    assert dispatcher_fixture_item["recommendedSliceSize"] == "complete"
    assert dispatcher_fixture_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-state-fixtures-refresh" in dispatcher_fixture_item["nextAction"]
    assert "GET /supervisor/runner-assignment-status-report" in dispatcher_fixture_item["relatedReports"]
    handoff_badges_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-badges-refresh")
    assert handoff_badges_item["status"] == "closed"
    assert handoff_badges_item["recommendedSliceSize"] == "complete"
    assert handoff_badges_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-badges-refresh" in handoff_badges_item["nextAction"]
    assert "GET /supervisor/runner-assignment-status-report" in handoff_badges_item["relatedReports"]
    handoff_status_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-status-refresh")
    assert handoff_status_item["status"] == "closed"
    assert handoff_status_item["recommendedSliceSize"] == "complete"
    assert handoff_status_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-status-refresh" in handoff_status_item["nextAction"]
    assert "GET /supervisor/runner-assignment-status-report" in handoff_status_item["relatedReports"]
    handoff_lifecycle_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-lifecycle-refresh")
    assert handoff_lifecycle_item["status"] == "closed"
    assert handoff_lifecycle_item["recommendedSliceSize"] == "complete"
    assert handoff_lifecycle_item["nextLane"] is None
    ...
    assert handoff_query_item["status"] == "closed"
    assert handoff_query_item["recommendedSliceSize"] == "complete"
    assert handoff_query_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-audit-query-refresh" in handoff_query_item["nextAction"]
    handoff_export_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-audit-export-refresh")
    assert handoff_export_item["status"] == "closed"
    assert handoff_export_item["recommendedSliceSize"] == "complete"
    assert handoff_export_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-audit-export-refresh" in handoff_export_item["nextAction"]
    handoff_download_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-audit-download-refresh")
    assert handoff_download_item["status"] == "closed"
    assert handoff_download_item["recommendedSliceSize"] == "complete"
    assert handoff_download_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-audit-download-refresh" in handoff_download_item["nextAction"]
    handoff_json_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-audit-json-refresh")
    assert handoff_json_item["status"] == "closed"
    assert handoff_json_item["recommendedSliceSize"] == "complete"
    ...
    assert handoff_json_validation_item["status"] == "closed"
    assert handoff_json_validation_item["recommendedSliceSize"] == "complete"
    assert handoff_json_validation_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-audit-json-validation-refresh" in handoff_json_validation_item["nextAction"]
    handoff_json_validation_fixtures_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-queue-handoff-audit-json-validation-fixtures-refresh")
    assert handoff_json_validation_fixtures_item["status"] == "closed"
    assert handoff_json_validation_fixtures_item["recommendedSliceSize"] == "complete"
    assert handoff_json_validation_fixtures_item["nextLane"] is None
    assert "do not requeue dispatcher-queue-handoff-audit-json-validation-fixtures-refresh" in handoff_json_validation_fixtures_item["nextAction"]
    dispatcher_cleanup_assignment_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-cleanup-assignment-closure-refresh")
    assert dispatcher_cleanup_assignment_item["status"] == "closed"
    assert dispatcher_cleanup_assignment_item["recommendedSliceSize"] == "complete"
    ...
    assert dispatcher_closed_lane_requeue_guard_item["status"] == "closed"
    assert dispatcher_closed_lane_requeue_guard_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_lane_requeue_guard_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-lane-requeue-guard-refresh" in dispatcher_closed_lane_requeue_guard_item["nextAction"]
    dispatcher_closed_source_guard_report_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-report-refresh")
    assert dispatcher_closed_source_guard_report_item["status"] == "closed"
    assert dispatcher_closed_source_guard_report_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_report_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-report-refresh" in dispatcher_closed_source_guard_report_item["nextAction"]
    dispatcher_closed_source_guard_drilldown_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-drilldown-refresh")
    assert dispatcher_closed_source_guard_drilldown_item["status"] == "closed"
    assert dispatcher_closed_source_guard_drilldown_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_drilldown_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-drilldown-refresh" in dispatcher_closed_source_guard_drilldown_item["nextAction"]
    dispatcher_closed_source_guard_rollup_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-rollup-refresh")
    assert dispatcher_closed_source_guard_rollup_item["status"] == "closed"
    assert dispatcher_closed_source_guard_rollup_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_rollup_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-rollup-refresh" in dispatcher_closed_source_guard_rollup_item["nextAction"]
    dispatcher_closed_source_guard_rollup_filter_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-rollup-filter-refresh")
    assert dispatcher_closed_source_guard_rollup_filter_item["status"] == "closed"
    assert dispatcher_closed_source_guard_rollup_filter_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_rollup_filter_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-rollup-filter-refresh" in dispatcher_closed_source_guard_rollup_filter_item["nextAction"]
    dispatcher_closed_source_guard_source_kind_summary_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-source-kind-summary-refresh")
    assert dispatcher_closed_source_guard_source_kind_summary_item["status"] == "closed"
    assert dispatcher_closed_source_guard_source_kind_summary_item["recommendedSliceSize"] == "complete"
    ...
    assert dispatcher_closed_source_guard_filter_presets_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_presets_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_presets_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-presets-refresh" in dispatcher_closed_source_guard_filter_presets_item["nextAction"]
    dispatcher_closed_source_guard_filter_counts_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-filter-counts-refresh")
    assert dispatcher_closed_source_guard_filter_counts_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_counts_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_counts_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-counts-refresh" in dispatcher_closed_source_guard_filter_counts_item["nextAction"]
    dispatcher_closed_source_guard_filter_empty_state_item = next(item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-filter-empty-state-refresh")
    assert dispatcher_closed_source_guard_filter_empty_state_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_empty_state_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-empty-state-refresh" in dispatcher_closed_source_guard_filter_empty_state_item["nextAction"]
    dispatcher_closed_source_guard_filter_empty_state_reset_item = next(
        item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-filter-empty-state-reset-refresh"
    )
    assert dispatcher_closed_source_guard_filter_empty_state_reset_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_reset_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_empty_state_reset_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-empty-state-reset-refresh" in dispatcher_closed_source_guard_filter_empty_state_reset_item["nextAction"]
    dispatcher_closed_source_guard_filter_empty_state_shortcuts_item = next(
        item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-filter-empty-state-shortcuts-refresh"
    )
    assert dispatcher_closed_source_guard_filter_empty_state_shortcuts_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcuts_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcuts_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-empty-state-shortcuts-refresh" in dispatcher_closed_source_guard_filter_empty_state_shortcuts_item["nextAction"]
    dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_item = next(
        item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-filter-empty-state-shortcut-counts-refresh"
    )
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-empty-state-shortcut-counts-refresh" in dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_item["nextAction"]
    dispatcher_closed_source_guard_filter_empty_state_shortcut_disabled_reasons_item = next(
        item for item in report["items"] if item["itemId"] == "dispatcher-closed-source-guard-filter-empty-state-shortcut-disabled-reasons-refresh"
    )
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_disabled_reasons_item["status"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_disabled_reasons_item["recommendedSliceSize"] == "complete"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_disabled_reasons_item["nextLane"] is None
    assert "do not requeue dispatcher-closed-source-guard-filter-empty-state-shortcut-disabled-reasons-refresh" in dispatcher_closed_source_guard_filter_empty_state_shortcut_disabled_reasons_item["nextAction"]
...
def test_disabled_provider_proofs_are_provider_specific_and_non_calling(tmp_path, monkeypatch) -> None:
    ...
    for proof in proofs:
        if proof["workerId"] == "local.ollama.disabled":
            assert proof["disabledReason"] == "ollama_enablement_authority_unresolved"
        else:
...
def test_local_evidence_packet_preview_is_non_mutating_and_bounded(tmp_path, monkeypatch) -> None:
    ...
    assert any("source-VM authority is selected" in boundary for boundary in packet["boundaries"])
...
def test_local_evidence_explanation_generation_is_non_mutating(tmp_path, monkeypatch) -> None:
    ...
    assert any("source-VM authority is selected" in boundary for boundary in explanation["boundaries"])
...
def test_runtime_evidence_export_returns_attempts_events_and_boundaries_without_mutation(tmp_path, monkeypatch) -> None:
    ...
    assert "GET /supervisor/epic-6-completion-audit-report" not in export["boundary"]["relatedSupervisorReports"]
...
def test_runner_assignment_status_report_reads_claimed_assignment_records(tmp_path, monkeypatch) -> None:
    ...
    assert queue_proof_backlog["classification"] == "closed"
    assert queue_proof_backlog["reasonCode"] == "backlog-closed"
    fixture_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-queue-state-fixtures-refresh")
    assert fixture_backlog["classification"] == "closed"
    assert fixture_backlog["reasonCode"] == "backlog-closed"
    handoff_badges_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-queue-handoff-badges-refresh")
    assert handoff_badges_backlog["classification"] == "closed"
    assert handoff_badges_backlog["reasonCode"] == "backlog-closed"
    handoff_status_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-queue-handoff-status-refresh")
    assert handoff_status_backlog["classification"] == "closed"
    assert handoff_status_backlog["reasonCode"] == "backlog-closed"
    handoff_lifecycle_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-queue-handoff-lifecycle-refresh")
    ...
    handoff_export_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-queue-handoff-audit-export-refresh")
    ...
    dispatcher_closed_source_guard_report_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-report-refresh")
    dispatcher_closed_source_guard_drilldown_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-drilldown-refresh")
    dispatcher_closed_source_guard_rollup_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-rollup-refresh")
    ...
    dispatcher_closed_source_guard_filter_counts_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-counts-refresh")
    dispatcher_closed_source_guard_filter_empty_state_backlog = next(row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-empty-state-refresh")
    dispatcher_closed_source_guard_filter_empty_state_reset_backlog = next(
        row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-empty-state-reset-refresh"
    )
    dispatcher_closed_source_guard_filter_empty_state_shortcuts_backlog = next(
        row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-empty-state-shortcuts-refresh"
    )
    dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_backlog = next(
        row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-empty-state-shortcut-counts-refresh"
    )
    dispatcher_closed_source_guard_filter_empty_state_shortcut_disabled_reasons_backlog = next(
        row for row in report["backlogCandidates"] if row["backlogItemId"] == "dispatcher-closed-source-guard-filter-empty-state-shortcut-disabled-reasons-refresh"
    )
    ...
    assert queue_proof_rows["queue-zero-runway-spillover-refresh"]["classification"] == "closed"
    assert queue_proof_rows["queue-zero-runway-spillover-refresh"]["reasonCode"] == "backlog-closed"
    ...
    assert queue_proof_rows["queue-zero-runway-standby-refresh"]["reasonCode"] == "backlog-closed"
    ...
    assert queue_proof_rows["queue-zero-runway-successor-refresh"]["reasonCode"] == "backlog-closed"
    queue_zero_runway_assignable = [
        row["backlogItemId"]
        for row in continuity["queueProofRows"]
        if row["backlogItemId"].startswith("queue-zero-runway-") and row["classification"] == "assignable"
    ]
    assert queue_zero_runway_assignable == ["queue-zero-runway-relay-refresh"]
    ...
    assert handoff_query_backlog["classification"] == "closed"
    assert handoff_query_backlog["reasonCode"] == "backlog-closed"
    assert handoff_export_backlog["classification"] == "closed"
    assert handoff_export_backlog["reasonCode"] == "backlog-closed"
    assert handoff_download_backlog["classification"] == "closed"
    ...
    assert handoff_json_validation_backlog["classification"] == "closed"
    assert handoff_json_validation_backlog["reasonCode"] == "backlog-closed"
    assert handoff_json_validation_fixtures_backlog["classification"] == "closed"
    ...
    assert dispatcher_closed_lane_requeue_guard_backlog["classification"] == "closed"
    assert dispatcher_closed_lane_requeue_guard_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_report_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_report_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_drilldown_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_drilldown_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_rollup_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_rollup_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_rollup_filter_backlog["classification"] == "closed"
    ...
    assert dispatcher_closed_source_guard_filter_presets_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_filter_presets_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_filter_counts_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_filter_counts_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_filter_empty_state_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_filter_empty_state_reset_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_reset_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcuts_backlog["classification"] == "closed"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcuts_backlog["reasonCode"] == "backlog-closed"
    assert dispatcher_closed_source_guard_filter_empty_state_shortcut_counts_backlog["classification"] == "closed"
    ...
    assert queue_proof_rows["dispatcher-queue-handoff-audit-export-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-queue-handoff-audit-download-refresh"]["classification"] == "closed"
    ...
    assert queue_proof_rows["dispatcher-queue-handoff-audit-json-validation-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-queue-handoff-audit-json-validation-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-queue-handoff-audit-json-validation-fixtures-refresh"]["classification"] == "closed"
    ...
    assert queue_proof_rows["dispatcher-closed-lane-requeue-guard-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-lane-requeue-guard-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-report-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-report-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-drilldown-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-drilldown-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-rollup-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-rollup-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-rollup-filter-refresh"]["classification"] == "closed"
    ...
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-presets-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-presets-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-counts-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-counts-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-reset-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-reset-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-shortcuts-refresh"]["classification"] == "closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-shortcuts-refresh"]["reasonCode"] == "backlog-closed"
    assert queue_proof_rows["dispatcher-closed-source-guard-filter-empty-state-shortcut-counts-refresh"]["classification"] == "closed"
...
def test_runner_assignment_status_report_closes_stale_ready_items_with_source_completion_evidence(tmp_path, monkeypatch) -> None:
    state_root = tmp_path / "codex-state"
    assignments_dir = state_root / "assignments"
    tasks_dir = state_root / "tasks"
    assignments_dir.mkdir(parents=True)
    ...
    tasks_dir.joinpath("read-only-evidence-polish.json").write_text(
        json.dumps(
            {
    ...
    monkeypatch.setenv("CODEX_WORKSPACE_STATE_ROOT", state_root.as_posix())
    client = _client(tmp_path, monkeypatch, "runner-assignment-status-closed-source.db")
...
def test_runner_assignment_status_report_surfaces_dispatch_handoff_evidence(tmp_path, monkeypatch) -> None:
    ...
    assert row["handoffCandidateStateCounts"] == {
        "active": 1,
        "blocked_authority": 1,
        "blocked_owned_active": 1,
        "closed": 9,
    ...
    unowned_prepared_path = tasks_dir / "unowned-prepared-handoff.json"
    unowned_prepared_path.write_text(
        json.dumps(
            {
                "task_id": "unowned-prepared-handoff",
                "title": "Unowned prepared handoff",
                "branch": "codex/unowned-prepared-handoff",
                "status": "active",
                "owner_updated_at": datetime.now(timezone.utc).isoformat(),
                "worktree_path": (state_root / "worktrees" / "unowned-prepared-handoff").as_posix(),
                "dispatch_handoffs": [
                    {
                        "lane": "unowned-prepared-handoff",
                        "next_command": "cd /tmp/unowned-prepared-handoff",
                        "generated_at": "2026-06-23T01:07:00Z",
    ...
    empty_counts_path.write_text(
        json.dumps(
            {
                "task_id": "empty-count-handoff",
                "title": "Empty count handoff",
                "branch": "codex/empty-count-handoff",
                "status": "active",
                "owner": "runner-a",
                "owner_updated_at": datetime.now(timezone.utc).isoformat(),
                "worktree_path": (state_root / "worktrees" / "empty-count-handoff").as_posix(),
                "dispatch_handoffs": [
                    {
                        "lane": "empty-count-handoff",
                        "next_command": "cd /tmp/empty-count-handoff",
    ...
    invalid_counts_path = tasks_dir / "invalid-count-handoff.json"
    invalid_counts_path.write_text(
        json.dumps(
            {
                "task_id": "invalid-count-handoff",
                "title": "Invalid count handoff",
                "branch": "codex/invalid-count-handoff",
                "status": "active",
                "owner": "runner-a",
                "owner_updated_at": datetime.now(timezone.utc).isoformat(),
                "worktree_path": (state_root / "worktrees" / "invalid-count-handoff").as_posix(),
                "dispatch_handoffs": [
                    {
                        "lane": "invalid-count-handoff",
                        "next_command": "cd /tmp/invalid-count-handoff",
                        "generated_at": "2026-06-23T01:15:00Z",
                        "candidate_state_counts": ["active"],
                    },
                    {
                        "lane": "invalid-count-handoff",
                        "next_command": "cd /tmp/invalid-count-handoff",
                        "generated_at": "2026-06-23T01:16:00Z",
                        "candidate_state_counts": {"active": "1"},
                    }
                ],
            }
    ...
    fallback_path = tasks_dir / "fallback-handoff.json"
    fallback_path.write_text(
        json.dumps(
            {
                "task_id": "fallback-handoff",
                "title": "Fallback handoff",
                "branch": "codex/fallback-handoff",
                "status": "active",
                "owner": "runner-a",
                "owner_updated_at": datetime.now(timezone.utc).isoformat(),
                "worktree_path": (state_root / "worktrees" / "fallback-handoff").as_posix(),
                "dispatch_handoffs": [
                    {
                        "lane": "fallback-handoff",
                        "next_command": "cd /tmp/fallback-handoff",
                        "generated_at": "2026-06-23T01:20:00Z",
                        "candidate_state_counts": {"active": 1},
                        "readiness": {"status": "passed", "command": "node ./scripts/codex-workspace.mjs doctor"},
                    },
                    {
                        "lane": "fallback-handoff",
                        "next_command": "cd /tmp/fallback-handoff-incomplete",
                        "generated_at": "2026-06-23T01:21:00Z",
                    },
                ],
            }
        )
    ...
    delivered_path.write_text(
        json.dumps(
            {
                "task_id": "delivered-handoff",
                "title": "Delivered handoff",
                "branch": "codex/delivered-handoff",
                "status": "active",
                "owner": "runner-a",
                "owner_updated_at": datetime.now(timezone.utc).isoformat(),
                "worktree_path": (state_root / "worktrees" / "delivered-handoff").as_posix(),
                "pr_number": 206,
                "dispatch_handoffs": [
                    {
    ...
    cleaned_path = tasks_dir / "cleaned-handoff.json"
    cleaned_path.write_text(
        json.dumps(
            {
                "task_id": "cleaned-handoff",
                "title": "Cleaned handoff",
                "branch": "codex/cleaned-handoff",
                "status": "closed",
                "owner": "runner-a",
                "owner_updated_at": datetime.now(timezone.utc).isoformat(),
                "worktree_path": (state_root / "worktrees" / "cleaned-handoff").as_posix(),
                "dispatch_handoffs": [
                    {
                        "lane": "cleaned-handoff",
                        "next_command": "cd /tmp/cleaned-handoff",
                        "generated_at": "2026-06-23T01:30:00Z",
                        "candidate_state_counts": {"closed": 1},
                    }
                ],
            }
        )
    ...
    counts_response = client.get("/supervisor/runner-assignment-status-report")

    assert counts_response.status_code == 200
    counts_report = counts_response.json()["data"]
    empty_counts_row = next(item for item in counts_report["workspaceAssignments"] if item["taskId"] == "empty-count-handoff")
    assert empty_counts_row["handoffCandidateStateCounts"] == {}
    assert empty_counts_row["handoffCandidateStateCountsStatus"] == "empty"
    invalid_counts_row = next(item for item in counts_report["workspaceAssignments"] if item["taskId"] == "invalid-count-handoff")
