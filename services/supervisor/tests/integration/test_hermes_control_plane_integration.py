...
from supervisor.api.schemas import HermesFollowUpWorkInputV1, HermesLedgerIngestRequest, HermesReviewHandoffRequest, HermesRoleCapabilityProvisionRequestV1
...
async def test_follow_up_admission_is_append_only_evidence_bound_and_idempotent(tmp_path):
    ...
    async with sessions() as session:
        ...
        admitted = await record_hermes_follow_up_work(session, HermesFollowUpWorkInputV1.model_validate(follow_up))
        assert admitted.followUpWorkId == "follow-up:one" and admitted.parentLaneRunId == "lane:1"
        assert await record_hermes_follow_up_work(session, HermesFollowUpWorkInputV1.model_validate(follow_up)) == admitted
        ...
        with pytest.raises(ValueError, match="idempotency"):
            await record_hermes_follow_up_work(session, HermesFollowUpWorkInputV1.model_validate(changed))
        ...
        with pytest.raises(ValueError, match="dedupe"):
            await record_hermes_follow_up_work(session, HermesFollowUpWorkInputV1.model_validate(duplicate))
        ...
        with pytest.raises(ValueError, match="evidence"):
            await record_hermes_follow_up_work(session, HermesFollowUpWorkInputV1.model_validate(unbound))
        ...
        with pytest.raises(ValueError, match="expired"):
            await record_hermes_follow_up_work(session, HermesFollowUpWorkInputV1.model_validate(expired))
    async with engine.begin() as connection:
