...
@pytest.mark.asyncio
async def test_dispatch_refuses_an_accepted_disclosure_after_its_source_expires(tmp_path) -> None:
    engine = create_async_engine(f"sqlite+aiosqlite:///{tmp_path / 'expired-claim.db'}")
    async with engine.begin() as connection:
        await connection.run_sync(Base.metadata.create_all)
    async with async_sessionmaker(engine, expire_on_commit=False)() as session:
        source = MemoryInboxSource(id="inbox-source:expired-claim", current_revision=2, lifecycle_state="Draft", retention_deadline_at=datetime.now(timezone.utc) + timedelta(minutes=1), deletion_state="None", policy_ref="memory-inbox-retention-v1")
        revision = MemoryInboxSourceRevision(id="inbox-source-revision:expired-claim", source_id=source.id, revision=2, lifecycle_state="Draft", actor_ref="operator:seed", audit_ref="audit:seed", policy_ref=source.policy_ref)
        session.add_all((source, revision)); await session.commit()
        disclosure = await present_processing_disclosure(session, source_id=source.id, expected_revision=2, idempotency_key="disclosure-key-expired-claim", actor_ref="operator:verified")
        await accept_processing_disclosure(session, disclosure_id=disclosure["disclosureId"], actor_ref="operator:verified")
        source.retention_deadline_at = datetime.now(timezone.utc) - timedelta(seconds=1); await session.commit()
        with pytest.raises(ValueError, match="dispatch_disclosure_stale"):
            await claim_processing_dispatch(session, disclosure_id=disclosure["disclosureId"], actor_ref="operator:verified")
    await engine.dispose()
