...
def test_work_packet_list_envelope_has_typed_list_and_optional_meta() -> None:
    ...
    assert get_args(WorkPacketListApiEnvelope.model_fields["data"].annotation) == (WorkPacketV0View,)

    with pytest.raises(ValidationError):
        WorkPacketListApiEnvelope.model_validate({"data": [], "unexpected": True})


def test_legacy_work_packet_list_route_uses_typed_envelope() -> None:
    assert _route("/work-packets").response_model is WorkPacketListApiEnvelope
