...
async def receive_quarantined_upload(session: AsyncSession, *, settings: Settings, chunks, actor_ref: str) -> str:
    ...
    manifest = MemoryInboxManifest(id=f"inbox-manifest:{uuid.uuid4().hex}", owner_revision_id=revision_id, copy_class="quarantine", store_ref=store_ref, creation_state="Planned", retention_class="source_retention", deletion_state="None")
