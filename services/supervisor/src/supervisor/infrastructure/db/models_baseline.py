...
class BaselineBase(DeclarativeBase):
    """Historical ORM metadata captured for migration revision 0001."""
...
class WorkItem(Base):
    ...
    audit_mode: Mapped[str] = mapped_column(String(16), default="none")
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow)
...
class SupervisorControl(Base):
    ...
    mode: Mapped[str] = mapped_column(String(16), default="running")
