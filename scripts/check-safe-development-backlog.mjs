...
for (const browserText of [
  ...
  'start: node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter empty state refresh"',
...
assertCondition(
  supervisorTests.includes('report_alignment_item["status"] == "closed"') &&
    ...
    supervisorTests.includes('"codex/dispatcher-closed-source-guard-filter-empty-state-refresh"') &&
    supervisorTests.includes('node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter empty state refresh"') &&
    ...
    supervisorTests.includes('dispatcher_closed_source_guard_filter_empty_state_item["status"] == "ready"'),
