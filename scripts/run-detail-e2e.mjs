...
const dbPath = (process.env.PLAYWRIGHT_E2E_DB_PATH ?? join(dataDir, `e2e-supervisor-detail-${process.pid}.db`)).replaceAll("\\", "/");
...
try {
  ...
  const dashboardCommand = process.platform === "win32" ? process.env.ComSpec ?? "cmd.exe" : "pnpm";
  const dashboardArgs = process.platform === "win32"
    ? ["/d", "/c", join(rootDir, "apps", "dashboard", "node_modules", ".bin", "next.CMD"), "dev", "--hostname", "127.0.0.1", "--port", dashboardPort]
    : ["--filter", "@kendall/dashboard", "exec", "next", "dev", "--hostname", "127.0.0.1", "--port", dashboardPort];
  ...
  const playwright = startProcess(
    ...
    [
      ...
      "dashboard.spec.ts",
      "--grep",
      "shows execution attempt evidence and disabled workspace boundaries",
    ],
    {
      cwd: rootDir,
      env: {
