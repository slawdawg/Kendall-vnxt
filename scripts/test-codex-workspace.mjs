...
try {
  ...
  test("assignment-report classifies safe backlog and workspace ownership without mutation", () => {
    ...
    assert(result.stdout.includes("- dispatcher-queue-handoff-audit-refresh | assignable"), result.stdout || result.stderr);
  ...
  test("claim-next advances to dispatcher queue handoff audit lane after completed recovery refresh", () => {
    ...
    try {
      ...
      assert(result.code === 0, result.stderr || result.stdout);
      assert(result.stdout.includes("claim candidate dispatcher-queue-handoff-audit-refresh"), result.stdout || result.stderr);
      assert(result.stdout.includes("branch codex/dispatcher-queue-handoff-audit-refresh"), result.stdout || result.stderr);
