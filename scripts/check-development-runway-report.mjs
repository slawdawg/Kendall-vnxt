...
for (const serviceText of [
  ...
  "Select this slice for dispatcher closed source guard filter empty-state shortcut reason keyboard loop work",
  ...
  "Start the authority approval scope readiness lane and keep real runtime execution disabled.",
...
for (const testText of [
  ...
  'node ./scripts/codex-workspace.mjs start "dispatcher closed source guard filter empty state shortcut reason keyboard loop refresh"',
  "pnpm run check:runner-assignment-status",
  "pnpm run check:safe-backlog",
  '"verification-runbook-hardening-slice"',
  '"authority-blocker-maintenance-slice"',
