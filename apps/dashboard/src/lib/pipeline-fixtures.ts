...
function buildLearnOutcome(
  ...
  return {
    ...
    decisionRecords: memoryProposals.map((proposal) => ({
      ...
      actor: proposal.decisionActorLabel ?? proposal.decisionActorId ?? "system",
...
function llmWikiRebuildReadyMemoryProposal(packetId: string, sourceRefs: string[], evidenceRef: string): MemoryProposalV0 {
  ...
  return {
    ...
    decisionActorId: "fixture:operator",
    decisionActorLabel: "Fixture Operator",
...
function humanGateActionRequest(
  ...
  return {
    ...
    requestDisplayLabel: action.type,
...
function rejectedHumanGateActionRequest(input: {
  ...
  return {
    ...
    requestDisplayLabel: "unknown_action",
    requestedByLabel: input.requestedByLabel,
    requestedAt: input.requestedAt,
    status: "rejected",
...
function cloneDensityPacket(packet: PipelineFixturePacket, ordinal: number): PipelineFixturePacket {
  ...
  return {
    ...
    humanGateActionRequests: packet.humanGateActionRequests.map((request) => ({
      ...request,
      requestId: remapId(request.requestId),
      packetId,
      actionId: remapId(request.actionId),
      decisionId: remapId(request.decisionId),
      evidenceRefs: request.evidenceRefs.map(remapId),
      rollbackPath: remapId(request.rollbackPath),
      stopLines: request.stopLines.map(remapId),
    })),
