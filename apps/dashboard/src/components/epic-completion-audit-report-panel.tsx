import type { EpicCompletionAuditItemView, EpicCompletionAuditReportView } from "@kendall/contracts";
...
function StatusChip({ value }: { value: string }) {
  const blocked = value.includes("blocked") || value.includes("needs");
  return (
    <span
    ...
    <section className="rounded-[1.75rem] border bg-[var(--panel)] p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-xl font-semibold">Completion status</h3>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          ["Epic complete", report.epicComplete ? "yes" : "no"],
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-2">
        <div>
          <h4 className="text-base font-semibold">Prepared</h4>
          <div className="mt-3 grid gap-3">
            {report.completedItems.map((item) => (
              <AuditItemCard key={item.itemId} item={item} />
            ))}
          </div>
        </div>
        <div>
          <h4 className="text-base font-semibold">Remaining</h4>
          <div className="mt-3 grid gap-3">
            {report.remainingItems.map((item) => (
              <AuditItemCard key={item.itemId} item={item} />
            ))}
          </div>
        </div>
