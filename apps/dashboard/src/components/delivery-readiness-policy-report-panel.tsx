import type { DeliveryReadinessPolicyItemView, DeliveryReadinessPolicyReportView } from "@kendall/contracts";

function formatTimestamp(value: string): string {
  return new Date(value).toLocaleString();
}

function PolicyItemCard({ item }: { item: DeliveryReadinessPolicyItemView }) {
  return (
  ...
  return (
    <section className="rounded-[0.5rem] border bg-[var(--panel)] p-4 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.32em] text-[var(--accent)]">Delivery readiness</p>
          <h3 className="mt-2 text-xl font-semibold">Review gate policy</h3>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-3">
        {[
          ["Status rules", String(report.statusPolicy.length)],
          ["Waiver rules", String(report.waiverPolicy.length)],
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="space-y-4">
          <div className="rounded-[0.5rem] border bg-[var(--surface)] p-4">
            <h4 className="text-base font-semibold">Status policy</h4>
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
              {report.statusPolicy.map((item) => (
          ...
          <div className="rounded-[0.5rem] border bg-[var(--surface)] p-4">
            <h4 className="text-base font-semibold">Waiver policy</h4>
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
              {report.waiverPolicy.map((item) => (
                <PolicyItemCard key={item.itemId} item={item} />
              ))}
            </div>
          </div>
