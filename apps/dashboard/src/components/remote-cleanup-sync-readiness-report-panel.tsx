import type { RemoteCleanupSyncPolicyItemView, RemoteCleanupSyncReadinessReportView } from "@kendall/contracts";

function formatTimestamp(value: string): string {
  return new Date(value).toLocaleString();
}

function PolicyCard({ item }: { item: RemoteCleanupSyncPolicyItemView }) {
...
export function RemoteCleanupSyncReadinessReportPanel({ report }: { report: RemoteCleanupSyncReadinessReportView }) {
  return (
    <section className="rounded-[1.75rem] border bg-[var(--panel)] p-6 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.32em] text-[var(--accent)]">Remote cleanup</p>
          <h3 className="mt-2 text-xl font-semibold">Remote sync readiness</h3>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ["Remote branches", report.remoteBranchDeletionApproved ? "approved" : "blocked"],
          ["Issues", report.issueSyncApproved ? "approved" : "blocked"],
          ["Stories", report.storyStatusSyncApproved ? "approved" : "blocked"],
          ["Remote mutation", report.remoteMutationApproved ? "approved" : "blocked"],
        ].map(([label, value]) => (
