...
import { BranchPreparationPanel } from "./branch-preparation-panel";
import { DeliveryCleanupPlanPanel } from "./delivery-cleanup-plan-panel";
import { DeliveryReadinessPanel } from "./delivery-readiness-panel";
import { EscalationPanel } from "./escalation-panel";
import { EvidenceOverviewPanel } from "./evidence-overview-panel";
import { ExecutionAttemptEvidencePanel } from "./execution-attempt-evidence-panel";
import { ExecutionRecipePanel } from "./execution-recipe-panel";
import { GreenGateReadinessPanel } from "./green-gate-readiness-panel";
import { LocalEvidencePanel } from "./local-evidence-panel";
import { LocalWorktreePlanPanel } from "./local-worktree-plan-panel";
import { MemoryProposalReviewPanel } from "./memory-proposal-review-panel";
import { RecipeGateAuditPanel } from "./recipe-gate-audit-panel";
import { RoutingPreviewPanel } from "./routing-preview-panel";
import { RuntimeEvidenceExportPanel } from "./runtime-evidence-export-panel";
...
import { SubscriptionLaunchReadinessPanel } from "./subscription-launch-readiness-panel";
import { WorkItemActions } from "./work-item-actions";
import { WorkItemHistory } from "./work-item-history";
import { WorkItemRetryHistory } from "./work-item-retry-history";
...
export async function loadWorkItemDetail(workItemId: string, signal?: AbortSignal) {
...
export function WorkItemDetailPage({
  ...
  workItemId: string;
  initialData?: WorkItemDetailData;
  lanAuthEnabled: boolean;
}) {
  const load = useCallback((signal: AbortSignal) => loadWorkItemDetail(workItemId, signal), [workItemId]);
  const { state: browserState, retry } = useAuthenticatedPageRead(load, [workItemId], () => false, !initialData);
  const state = initialData ? { kind: "ready" as const, data: initialData, error: null } : browserState;
  ...
  return (
    <Shell lanAuthEnabled={lanAuthEnabled} navStats={navStats}>
